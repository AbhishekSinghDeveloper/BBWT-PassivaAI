...
public interface IRqbQueryProcessorDefault
{
    string SqlAliasCleanup(string sqlAlias);

    string SqlCodeAliasesCleanup(string sqlCode);

    string SqlCodeCleanup(string sqlCode);

    SqlQueryValidateResult ValidateSqlCode(string sqlCode);
    ...
    string GetSqlCodeSelectStatement(string sqlCode);
    ...
    string ProcessSqlCodeOrganizationBasedFiltering(string sqlCode,
        IEnumerable<QuerySchemaColumn> columns, QueryFilterMode? filterMode);
