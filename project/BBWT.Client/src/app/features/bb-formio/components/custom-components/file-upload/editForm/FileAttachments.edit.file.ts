...
export default [
    {
        ...
        data: {
            custom() {
                return "s3";
                // TODO: add others Providers, custom logic needed
                return _.map(Providers.getProviders("storage"), (storage, key) => {
    ...
    //         "Regular template variables are available (`data`, `component`, `user`, `value`, `moment` etc.), also `fileName`, `guid` variables are available. `guid` part must be present, if not found in template, will be added at the end.",
    //     weight: 25
    // },
    // {
    //     type: "checkbox",
