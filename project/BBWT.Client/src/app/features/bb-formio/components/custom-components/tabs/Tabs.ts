...
import { FormioComponentSelectors } from "@features/bb-formio/classes/pdf-generator/formio-components-selectors";

const NestedComponent = Components.components.nested;

export default class TabsComponent extends (NestedComponent as any) {

  public static editForm = editForm;

  static schema(...extend) {
    return NestedComponent.schema({
      label: "Multi-User Tabs",
      type: "state-tabs",
      input: false,
      key: "tabs",
      persistent: false,
  ...
  get className() {
    ...
    if(!className?.includes(FormioComponentSelectors.Tabs)) {
      className = `${className} ${FormioComponentSelectors.Tabs}`;
    }
    return className;
  }
  ...
  init() {
    ...
    _.each(this.component.components, (tab, index) => {
      ...
      if (!!this.builderMode) {
        // attach a new property to the Tabs component
        tab.state = TabState.Edit;
      }

      if (tab.state == TabState.Hide && index == this.currentTab) {
        this.currentTab = -1;
      } else if (this.currentTab == -1 && tab.state != TabState.Hide) {
      ...
      _.each(tab.components, (comp) => {
        ...
        if (tab.state == TabState.ReadOnly) {
...
Components.addComponent("state-tabs", TabsComponent);
