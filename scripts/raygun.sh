...
deployment="{\"apiKey\":\"${RAYGUN_API_KEY}\",\"version\": \"${DEPLOYMENT_VERSION}\",\"ownerName\": \"${DEPLOYED_BY}\",\"emailAddress\": \"${EMAIL_ADDRESS}\",\"scmIdentifier\": \"${GIT_HASH}\",\"comment\": \"${DEPLOYMENT_NOTES}\"}"

if ! curl -H "Content-Type: application/json" -d "$deployment" -f $url; then
